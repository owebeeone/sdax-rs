...
| `V-SERVICE-UNBOUNDED` | `Shutdown::unbounded()` ⇒ every service in the complete component/template declaration tree declares `stop_within`, even below bounded child scopes (T7a); findings use root-relative paths in depth-first declaration order | P-11 |
...
| **OD-NESTED-STOP-BOUND** | child services under an unbounded root can stop before a child budget starts | Validate the complete component/template declaration tree at build time and require every service to declare `stop_within`. Preserve T7a and all runtime timing. Report every registration by root-relative path, including unspawned templates. | 2026-09-08 | Owner accepted the whole-tree validation plan; a bounded child budget alone does not protect this interval. |
