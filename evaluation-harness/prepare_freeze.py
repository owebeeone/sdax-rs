...
def reduce_context(text):
    sections=re.split(r"(?=^## )",text,flags=re.M)
    sections=[s for s in sections if s.startswith("## ")]
    if len(sections)<2:
        raise ValueError("Reference must contain at least two complete level-two sections")
    return "\n\n".join(s.rstrip() for s in sections[:2])+"\n"
