...
fn behavior() {
    ...
    for n in [6, 19] {
        ...
        for event in [
            format!("ack:{}", 2 * n),
            format!("unknown:{}", 2 * n + 1),
            format!("undo:{}", 2 * n + 900),
            format!("reconcile:{}", 2 * n + 1),
        ] {
            assert_eq!(count(&ev, &event), 1, "{ev:?}");
        }
        assert_eq!(ev.len(), 4, "{ev:?}");
        assert_eq!(r.ambiguous.len(), 1, "{r}");
        assert_eq!(r.cleanup_failures.len(), 1, "{r}");
        let s = candidate::boundary(r).unwrap_err();
        for word in ["pending", "dispatch", "ambiguous"] {
